...
def _create_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📊 إنشاء منشور نتائج", callback_data="results:start")],
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إنشاء منشور نتائج بطولة (تدفّق خطوتين: معلومات ثم جدول)
    if action == "results":
        if token == "start":
            context.bot_data["awaiting_results"] = "info"
            context.bot_data["results_info"] = {}
            await q.edit_message_text("📊 إنشاء منشور نتائج…")
            await context.bot.send_message(ADMIN, RESULTS_INFO_FORM)
        return
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.bot_data.get("awaiting_tournament"):
        ...
        return
    # تدفّق نتائج البطولة — الخطوة 1: معلومات البطولة
    res_state = context.bot_data.get("awaiting_results")
    if res_state == "info":
        context.bot_data["awaiting_results"] = "data"
        info = {} if text.strip() in ("تخطي", "skip", "-") else _parse_results_info(text)
        context.bot_data["results_info"] = info
        await update.message.reply_text(RESULTS_DATA_FORM)
        return
    # تدفّق نتائج البطولة — الخطوة 2: جدول النتائج (نصًا يدويًا؛ الإكسل عبر on_document)
    if res_state == "data":
        rows = results_parser.parse_manual(text)
        if not rows:
            await update.message.reply_text(
                "⚠️ لم أفهم أي صف نتائج. تأكد من الصيغة:\n"
                "الترتيب،الاسم،النقاط،التصنيف\n"
                "أو أرسل ملف إكسل (.xlsx) كملف مباشرة.")
            return
        context.bot_data["awaiting_results"] = None
        await update.message.reply_text(f"جارٍ تجهيز منشور النتائج ({len(rows)} لاعبًا)…")
        await _finish_results(context, rows)
        return
...
# ---------------- إدخال نتائج بطولة (معلومات + جدول) ----------------
RESULTS_INFO_FORM = (
    "📊 الخطوة 1 من 2 — معلومات البطولة:\n"
    "أرسل التفاصيل بهذا الشكل (انسخ السطور وعدّل القيم):\n\n"
    "الاسم: بطولة الجزائر المفتوحة\n"
    "التاريخ: 28 يونيو 2026\n"
    "النظام: سويسري 7 جولات 90+30\n"
    "المكان: قسنطينة\n"
    "الرابط: https://chess-results.com/...\n\n"
    "(الرابط اختياري — يظهر في نص المنشور كرابط للنتائج الكاملة)\n"
    "أو أرسل: تخطي  — لمتابعة بدون هذه التفاصيل."
)
RESULTS_DATA_FORM = (
    "📋 الخطوة 2 من 2 — أرسل جدول النتائج:\n\n"
    "🔹 الطريقة 1: أرسل ملف إكسل (.xlsx) كما هو، مثلاً المُصدَّر من موقع chess-results.com.\n"
    "🔹 الطريقة 2: اكتبه يدويًا، سطر لكل لاعب بهذا الترتيب:\n"
    "الترتيب،الاسم،النقاط،التصنيف\n\n"
    "مثال:\n"
    "1،علاء الدين بورلناس،7،2345\n"
    "2،محمد بن صالح،6.5،2210\n"
    "3،يوسف العماري،6\n\n"
    "(التصنيف اختياري، اتركه فارغًا إن لم تعرفه)"
)
_RFIELDS = {"الاسم": "name", "التاريخ": "date", "اليوم والوقت": "date", "اليوم": "date",
            "النظام": "system", "المكان": "place", "الموقع": "place", "الرابط": "link"}


def _parse_results_info(text):
    d = {}
    for line in text.splitlines():
        sep = ":" if ":" in line else ("：" if "：" in line else None)
        if not sep:
...
async def cmd_results(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.bot_data["awaiting_results"] = "info"
    context.bot_data["results_info"] = {}
    await update.message.reply_text(RESULTS_INFO_FORM)


async def _finish_results(context, rows):
    """يبني صفحة/صفحات النتائج ويرسلها للموافقة، مهما كان مصدر الجدول (نص أو إكسل)."""
    info = context.bot_data.get("results_info") or {}
    pages = await asyncio.to_thread(content.build_results_post, info, rows)
    for img, caption, comment in pages:
        await deliver(context, img, caption, comment, auto=False, label="نتائج بطولة")


async def on_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """استقبال ملف إكسل لنتائج بطولة (الخطوة 2 من تدفّق النتائج فقط)."""
    if not _is_admin(update) or not update.message.document:
        return
    if context.bot_data.get("awaiting_results") != "data":
        return
    doc = update.message.document
    fname = (doc.file_name or "").lower()
    if not fname.endswith((".xlsx", ".xlsm")):
        await update.message.reply_text("أرسل ملف إكسل بصيغة .xlsx، أو اكتب النتائج يدويًا كنص (انظر التعليمات أعلاه).")
        return
    path = os.path.join(CARDS_DIR, f"res_{uuid.uuid4().hex[:8]}.xlsx")
    ...
    try:
        rows = await asyncio.to_thread(excel_results.parse_excel, path)
    except Exception as e:
        log.warning("فشل قراءة ملف النتائج: %s", e)
        await update.message.reply_text(
            "تعذّر قراءة الملف. تأكد أنه ملف .xlsx سليم وفيه أعمدة الاسم/النقاط.")
        return
    if not rows:
        await update.message.reply_text(
            "لم أجد جدول نتائج واضحًا في الملف (أحتاج عمود اسم على الأقل). "
            "جرّب ملفًا آخر، أو اكتب النتائج يدويًا كنص.")
        return
    context.bot_data["awaiting_results"] = None
    await update.message.reply_text(f"جارٍ تجهيز منشور النتائج ({len(rows)} لاعبًا)…")
    await _finish_results(context, rows)
...
_COMMANDS = [
    ...
    BotCommand("results", "نشر نتائج بطولة"),
