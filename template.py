...
# ---------------- بطاقة نتائج بطولة (جدول) ----------------
RANK_COLORS = {1: (212, 175, 55), 2: (192, 192, 192), 3: (176, 124, 68)}  # ذهبي/فضي/برونزي


def _fit_text(d, t, f, max_w):
    """يقصّ النص ويضيف «…» إن لم يتّسع داخل max_w (لأسماء اللاعبين الطويلة)."""
    if not t:
        return t
    if text_w(d, t, f) <= max_w:
        return t
    while t and text_w(d, t + "…", f) > max_w:
        t = t[:-1]
    return (t + "…") if t else "…"


def create_results_card(tournament_name, info_line, rows, page=1, total_pages=1,
                        output="output/results.png"):
    """بطاقة جدول نتائج بطولة (حتى ~15 صفًا في الصفحة الواحدة).
    rows: قائمة dict فيها rank/name/points/rating (rating قد تكون فارغة).
    عند تعدّد الصفحات استدعِ الدالة مرّة لكل صفحة (15 لاعبًا تقريبًا لكل صورة)."""
    cat = CATS["results"]
    tournament_name = strip_emoji(tournament_name) or "بطولة"
    info_line = strip_emoji(info_line)
    img = BACKGROUNDS["diag"](W, H).convert("RGBA")
    d = ImageDraw.Draw(img)
    accent = cat["color"]

    # الترويسة
    d.rectangle([0, 0, W, HEADER_H], fill=(20, 18, 14))
    d.line([0, HEADER_H, W, HEADER_H], fill=accent, width=3)
    label = cat["label"] if total_pages <= 1 else f"{cat['label']} {page}/{total_pages}"
    pf = font(F_BOLD, 34); pw = text_w(d, label, pf)
    pill_w, pill_h = pw + 56, 60; px0, py0 = 45, (HEADER_H - pill_h)//2
    d.rounded_rectangle([px0, py0, px0+pill_w, py0+pill_h], radius=pill_h//2, fill=accent)
    dtext(d, (px0+pill_w/2, py0+pill_h/2), label, pf, "mm", _pill_text_color(accent))
    avail = (W - 45) - (px0 + pill_w + 24)
    hsize = 44; hf = font(F_BOLD, hsize)
    while hsize > 22 and text_w(d, tournament_name, hf) > avail:
        hsize -= 2; hf = font(F_BOLD, hsize)
    dtext(d, (W-45, HEADER_H//2), tournament_name, hf, "rm", accent)

    # سطر المعلومات (التاريخ / النظام / المكان)
    info_h = 56
    if info_line:
        dtext(d, (W//2, HEADER_H + info_h//2), info_line, font(F_BOLD, 27), "mm", MUTED)
    table_top = HEADER_H + (info_h if info_line else 14)

    # أعمدة الجدول (من اليمين لليسار) — عمود "الدولة/النادي" يظهر فقط إن وُجدت بيانات fed
    has_fed = any((row.get("fed") or "").strip() for row in rows)
    c_left, c_right = 50, W - 50; cw = c_right - c_left
    GAP = 14
    WIDTHS = {"rank": 88, "fed": 92, "points": 120, "rating": 140}
    HEADERS = {"rank": "الترتيب", "name": "الاسم", "fed": "الدولة/النادي",
              "points": "النقاط", "rating": "التصنيف"}
    order = ["rank", "name"] + (["fed"] if has_fed else []) + ["points", "rating"]
    fixed_sum = sum(WIDTHS[k] for k in order if k != "name")
    WIDTHS["name"] = cw - fixed_sum - GAP * (len(order) - 1)

    bounds = {}; cur_r = c_right
    for k in order:
        cur_l = cur_r - WIDTHS[k]
        bounds[k] = (cur_l, cur_r)
        cur_r = cur_l - GAP
    NAME_W = WIDTHS["name"]

    th_top = table_top + 6; th_h = 50
    d.rounded_rectangle([c_left, th_top, c_right, th_top+th_h], radius=10, fill=(26, 23, 18))
    hf2 = font(F_BOLD, 26)
    for k in order:
        xl, xr = bounds[k]
        if k == "name":
            dtext(d, (xr, th_top+th_h/2), HEADERS[k], hf2, "rm", accent)
        else:
            dtext(d, ((xl+xr)/2, th_top+th_h/2), HEADERS[k], hf2, "mm", accent)

    # صفوف البيانات
    data_top = th_top + th_h + 8
    data_bottom = H - BOTTOM_H - 16
    avail_h = max(data_bottom - data_top, 0)
    row_h = max(34, min(54, avail_h // max(len(rows), 1))) if rows else 0
    block_h = row_h * len(rows)
    y0 = data_top + max(0, (avail_h - block_h)//2)

    name_f = font(F_BOLD, 30); cell_f = font(F_REG, 28); rank_f = font(F_BOLD, 26)
    fed_f = font(F_REG, 22)
    x_rank_l, x_rank_r = bounds["rank"]; x_name_l, x_name_r = bounds["name"]
    x_pts_l, x_pts_r = bounds["points"]; x_rtg_l, x_rtg_r = bounds["rating"]
    for i, row in enumerate(rows):
        ry0 = y0 + i*row_h; ry1 = ry0 + row_h; cy = (ry0+ry1)/2
        if i % 2 == 0:
            d.rectangle([c_left, ry0, c_right, ry1], fill=(24, 21, 16))
        try:
            rk = int(str(row.get("rank", "")).strip())
        except (ValueError, TypeError):
            rk = None
        rk_txt = str(row.get("rank") or (i + 1))
        rank_cx = (x_rank_l + x_rank_r) / 2
        if rk in RANK_COLORS:
            rr = min(20, row_h//2 - 4)
            d.ellipse([rank_cx-rr, cy-rr, rank_cx+rr, cy+rr], fill=RANK_COLORS[rk])
            dtext(d, (rank_cx, cy), rk_txt, rank_f, "mm", (20, 18, 14))
        else:
            dtext(d, (rank_cx, cy), rk_txt, rank_f, "mm", WHITE)
        name_txt = _fit_text(d, str(row.get("name", "")), name_f, NAME_W - 10)
        dtext(d, (x_name_r, cy), name_txt, name_f, "rm", WHITE)
        if has_fed:
            xl, xr = bounds["fed"]
            fed_txt = _fit_text(d, str(row.get("fed") or "-"), fed_f, WIDTHS["fed"] - 6)
            dtext(d, ((xl+xr)/2, cy), fed_txt, fed_f, "mm", MUTED)
        dtext(d, ((x_pts_l+x_pts_r)/2, cy), str(row.get("points") or "-"), cell_f, "mm", WHITE)
        dtext(d, ((x_rtg_l+x_rtg_r)/2, cy), str(row.get("rating") or "-"), cell_f, "mm", MUTED)
        if i < len(rows) - 1:
            d.line([c_left, ry1, c_right, ry1], fill=(40, 36, 28), width=1)

    if not rows:
        dtext(d, (W//2, (data_top+data_bottom)//2), "لا توجد بيانات", font(F_REG, 32), "mm", MUTED)

    # الشريط السفلي
    d.line([45, H-BOTTOM_H, W-45, H-BOTTOM_H], fill=(70, 62, 50), width=2)
    logo = Image.open(LOGO).convert("RGBA"); lh3 = 100
    logo = logo.resize((int(logo.width*lh3/logo.height), lh3), Image.LANCZOS)
    img.alpha_composite(logo, (45, H-BOTTOM_H + (BOTTOM_H-lh3)//2))
    foot = f"عدد المشاركين: {len(rows)}" if total_pages <= 1 else f"صفحة {page} من {total_pages}"
    dtext(d, (W-45, (H-BOTTOM_H) + BOTTOM_H//2), foot, font(F_BOLD, 36), "rm", WHITE)
